...
class gclda_model:
	...
	# -----------------------------------------------------------------------
	def __init__(self, dat, nt = 100, nr = 2, alpha = .1, beta = .01, gamma = .01, delta = 1.0, dobs=50.0, roi=50.0, symmetric = False, seed_init = 1):
		'''
	...
	# ----------------------------------------------------------------------------------------
	def printTopicFigures(self, outputdir):
		# If output directory doesn't exist, make it
		if not os.path.isdir(outputdir):
			os.mkdir(outputdir)
		...
		backgroundvals = self.dat.peak_vals[range(1,len(self.dat.peak_vals)-1,10),:]
